...
editor_init :: proc(editor: ^Editor, prj: ^Project) -> bool {
	editor.project = prj
	// Tracks register themselves from the layout, so only the spacing lives here.
	editor.grid = {
		gap = MAIN_GAP,
		pad = MAIN_PAD,
	}
...
editor_frame :: proc(editor: ^Editor, ctx: ^Ctx) {
	...
	if clay.UI(clay.ID("editor"))(
	{
		...
		grid_update(ctx, &editor.grid)

		if clay.UI(clay.ID("main_area"))(
		{layout = grid_container_layout(&editor.grid, "main_area", .TopToBottom)},
		) {
